...
using SiMay.RemoteMonitor.Controls.Entitys;
...
{
    [ControlSource(10, "文件管理", "FileManagerJob", "FileManager")]
    public partial class FileManager : Form, IControlSource
    {
        private const Int32 IDM_READ_LOG = 1000;
        ...
        public FileManager(MessageAdapter adapter)
        {
            _adapter = adapter;
        ...
        {
            this.Text = "//远程文件管理 连接中....";
            this.Initialize();
        }

        private void Initialize()
        {
            string downPath = Path.Combine(Application.StartupPath, "download");
            ...
            InsertMenu(sysMenuHandle, 8, MF_BYPOSITION, IDM_READ_LOG, "查看文件管理异常日志");
        }
            ...
            if (m.Msg == WM_SYSCOMMAND)
            {
                ...
                switch (m.WParam.ToInt64())
                {
                    case IDM_READ_LOG:
                        ...
                        {
                            MessageBox.Show("无日志!", "提示", 0, MessageBoxIcon.Exclamation);
                        }
                        break;
                }
            }
        ...
        public void FixedRemoteFileList(SessionHandler session)
        {
            ...
            if (!fileLst.IsSccessed)
                MessageBox.Show(fileLst.Message, "提示", 0, MessageBoxIcon.Exclamation);
        ...
        private void OnSelectRemoteOpenFile()
        {
            if (this.fileList.SelectedItems.Count != 0)
            {
                ...
                if (file.FileType == FileItemType.File)
                {
                    ...
                    {
                        MessageBox.Show("仅支持打开512KB以下的文件!", "提示", 0, MessageBoxIcon.Exclamation);
                        return;
                    }
        ...
        public void OpenRemoteText(SessionHandler session)
        {
            ...
            if (!text.IsSuccess)
            {
                MessageBox.Show("远程文件打开失败!", "提示", 0, MessageBoxIcon.Exclamation);
                return;
            }
            var tmp = Path.Combine(Application.StartupPath, "tmp");
        ...
        private void 新建文件夹ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (txtRemotedirectory.Text.IsNullOrEmpty())
            {
                MessageBox.Show("根目录不能创建文件夹!", "提示", 0, MessageBoxIcon.Exclamation);
                return;
            }
            ...
            if (dlg.ShowDialog() == DialogResult.OK && !dlg.Value.IsNullOrEmpty())
            {
                foreach (FileListViewItem item in fileList.Items)
                {
                    ...
                    {
                        MessageBox.Show("已与现有文件重名!", "提示", 0, MessageBoxIcon.Exclamation);
                        return;
                    }
                }
                if (FileHelper.VerifyFileName(dlg.Value))
                {
                    if (!this.RemoteCreateDirectory(dlg.Value))
                    {
                        MessageBox.Show("文件夹创建失败!路径", "提示", 0, MessageBoxIcon.Error);
                    }
                }
                else
                {
                    MessageBox.Show("文件名不能包含 | / \\ * ? \" < > : 等特殊符号!", "提示", 0, MessageBoxIcon.Error);
                }
            }
        }
        ...
        private bool RemoteCreateDirectory(string path, bool noCallback = false)
        {
            ...
            _adapter.SendAsyncMessage(MessageHead.S_FILE_CREATE_DIR,
                ...
                {
                    DirectoryName = Path.Combine(txtRemotedirectory.Text, path),
        ...
        public void DirectoryCreateFinishHandler(SessionHandler session)
        {
            ...
            if (response.IsSuccess)
                ...
                MessageBox.Show("文件夹创建失败!", "提示", 0, MessageBoxIcon.Error);
        }
        ...
        private void 重命名ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (files.Any())
            {
                ...
                {
                    MessageBox.Show("磁盘不能作为重命名的对象!", "警告", 0, MessageBoxIcon.Error);
                    return;
                }
                EnterForm dlg = new EnterForm();
        ...
        public void ReNameFinishHandler(SessionHandler session)
        {
            ...
            if (file.IsSuccess)
            {
                ...
                MessageBox.Show("文件重命名失败!", "提示", 0, MessageBoxIcon.Error);
        }
        private void 复制文件ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (files.Any(c => c.FileType == FileItemType.Disk))
            {
                MessageBox.Show("磁盘对象不支持复制!", "提示", 0, MessageBoxIcon.Error);
                return;
            }
            if (files.Length > 0)
        ...
        private void toolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("请选择要复制的文件!", "警告", 0, MessageBoxIcon.Error);
                return;
            }
            if (txtRemotedirectory.Text.IsNullOrEmpty())
            {
                MessageBox.Show("当前路径不能粘贴!", "警告", 0, MessageBoxIcon.Error);
                return;
            }
        ...
        public void CopyFileFinishHandler(SessionHandler session)
        {
            ...
            if (fails.ExceptionFileNames.Length > 0)
                MessageBox.Show($"文件复制完成，但有部分文件复制操作异常:{string.Join(", ", fails.ExceptionFileNames)},请检查!", "提示", 0, MessageBoxIcon.Exclamation);
            else
                MessageBox.Show("文件复制完成!", "提示", 0, MessageBoxIcon.Exclamation);
        }
        private async void downloadMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var root = txtRemotedirectory.Text;
            ...
            foreach (var fileItem in this.GetSelectFiles())
            {
                ...
                {
                    MessageBox.Show("当前选项中包含了暂未支持传输的部分!", "提示", 0, MessageBoxIcon.Exclamation);
                    return;
                }
        ...
        private async void 文件ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (txtRemotedirectory.Text.IsNullOrEmpty())
            {
                MessageBox.Show("当前路径不能上传!", "提示", 0, MessageBoxIcon.Error);
                return;
            }
        ...
        private async void 文件夹ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (dlg.SelectedPath != "" && result == DialogResult.OK)
            {
                if (dlg.SelectedPath == Path.GetPathRoot(dlg.SelectedPath))
                    MessageBox.Show("暂不支持根目录的发送,请重新选择文件夹!", "提示", 0, MessageBoxIcon.Error);
                else
                {
                    this._transferMode = null;
        ...
        private void linkLabel1_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            if (dlg.SelectedPath != "")
            {
                IniConfigHelper.SetValue("RemoteFile", "SavaFilePath", dlg.SelectedPath, Application.StartupPath + @"\SiMayConfig.ini");
