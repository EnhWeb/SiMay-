...
namespace SiMay.RemoteMonitor.Controls
{
    ...
    public partial class ScreenManager : Form, IControlSource
    {
        ...
        private string _stitle = "//远程桌面 当前桌面帧率 {0}/秒 总流量 {1} KB";
        ...
        {
            this.Text = string.Format(_stitle, "0", "0.0");
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            if (_continueTask)
            {
                this.Text = string.Format(_stitle, _recvImgCount.ToString(), (_traffic / (float)1024).ToString("0.00"));
                _recvImgCount = 0;
            }
        }
        ...
        private void Init()
        {
            ...
            InsertMenu(sysMenuHandle, 8, MF_BYPOSITION, IDM_SCREENMON, "屏幕监控");
            InsertMenu(sysMenuHandle, 9, MF_BYPOSITION, IDM_FULLSCREEN, "全屏监控");
            InsertMenu(sysMenuHandle, 10, MF_SEPARATOR, 0, null);
            InsertMenu(sysMenuHandle, 11, MF_BYPOSITION, IDM_KEYMOUSE_CTRL, "鼠标控制");
            InsertMenu(sysMenuHandle, 12, MF_BYPOSITION, IDM_LOCK_MOUSEKEY, "锁定鼠键");
            InsertMenu(sysMenuHandle, 13, MF_BYPOSITION, IDM_BLACKSCREEN, "屏幕黑屏");
            InsertMenu(sysMenuHandle, 14, MF_BYPOSITION, IDM_SAVESCREEN, "保存快照");
            InsertMenu(sysMenuHandle, 15, MF_SEPARATOR, 0, null);
            InsertMenu(sysMenuHandle, 16, MF_BYPOSITION, IDM_FULL_DIFFER, "差异扫描");
            InsertMenu(sysMenuHandle, 17, MF_BYPOSITION, IDM_DIFFER, "逐行扫描");
            InsertMenu(sysMenuHandle, 18, MF_SEPARATOR, 0, null);
            InsertMenu(sysMenuHandle, 19, MF_BYPOSITION, IDM_1X, "1位黑白");
            InsertMenu(sysMenuHandle, 20, MF_BYPOSITION, IDM_4X, "4位彩色");
            InsertMenu(sysMenuHandle, 21, MF_BYPOSITION, IDM_16X, "16位高彩");

            CheckMenuItem(sysMenuHandle, IDM_FULLSCREEN, MF_CHECKED);
            ...
            if (m.Msg == WM_SYSCOMMAND)
            {
                ...
                switch (m.WParam.ToInt32())
                {
                    case IDM_SCREENMON:
                        ...
                        CheckMenuItem(sysMenuHandle, IDM_FULLSCREEN, MF_UNCHECKED);
                    ...
                    case IDM_SAVESCREEN:
                        ...
                        MessageBox.Show("快照已保存到:" + Application.StartupPath + "\\" + fileName, "提示", 0, MessageBoxIcon.Exclamation);
                        break;
        ...
        {
            var bitinfo = session.CompletedBuffer.GetMessageEntity<ScreenBitInfoPack>();
            //原始图像尺寸
