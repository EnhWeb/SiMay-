...
{
    [ControlSource(50, "注册表管理", "RegEditManagerJob", "RegEditManager")]
    public partial class RegistryManager : Form, IControlSource
    {
        ...
        public void CreateSubkeyFinishHandler(SessionHandler session)
        {
            ...
            if (result.Result)
            {
                ...
                MessageBox.Show(result.Value + " 创建成功!", "提示", 0, MessageBoxIcon.Exclamation);
            }
        ...
        private void 新建ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (dlg.ShowDialog() == DialogResult.OK || dlg.Value != "")
            {
                if (dlg.Value == "")
                {
                    MessageBox.Show("子项名称不能为空!", "提示", 0, MessageBoxIcon.Exclamation);
                    return;
                }
                ...
                foreach (TreeNode item in node.Nodes)
                {
                    ...
                    {
                        MessageBox.Show(dlg.Value + "已存在,创建失败!", "提示", 0, MessageBoxIcon.Exclamation);
                        return;
                    }
                }
        ...
        private void 删除ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (valueView.SelectedItems.Count < 1)
            {
                MessageBox.Show("请选择要删除的对象!", "提示", 0, MessageBoxIcon.Exclamation);
                return;
            }
